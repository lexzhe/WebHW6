...
public class ArticleRepositoryImpl implements ArticleRepository {
    ...
    public List<Article> findAll() {
        ...
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM Article WHERE hidden='0' ORDER BY creationTime DESC")) {
                try (ResultSet resultSet = statement.executeQuery()) {
                    Article article;
...
//    }
    ...
    public void setHidden(long id, boolean hidden) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("UPDATE `Article` SET `hidden` = ? WHERE `Article`.`id` = ?")) {
                statement.setLong(1, hidden ? 1 : 0);
